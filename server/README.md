...
| `CORS_ORIGINS` | `http://localhost:5173,http://localhost:3000` | Extra allowed origins (comma-separated, exact match), also used by the CSRF check. The web dev server is on `:5174`, so add it locally. |
...
5. CORS
6. session
7. `/webhooks/*` (payment provider callbacks, outside CSRF)
8. CSRF (Origin/Referer check on unsafe methods under `/api/v1`)
9. rate limit
10. `/api/v1` routes (`routes/index.ts`: public routes, then `routes/protected.ts`, then go-live)
11. error handler (`middleware/error.ts` maps known `lib/` errors to 4xx and everything else to a 500 with a request id)
