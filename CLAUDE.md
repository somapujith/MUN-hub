...
UI/pages (mun-hub-93 session): homepage (`app/page.tsx`), `/muns` search+filter (`app/muns/`), MUN detail (`app/mun/[slug]/`) all live against real `searchMuns`/`getMarketplaceFacets`/`getMunBySlug`. `app/not-found.tsx` added. Components: `mun-card`/`mun-card-grid`/`mun-hero`/`committee-list`/`registration-product-card`, `marketplace/{search-bar,filter-sidebar}`. Design-critic ran 3 verification rounds (score progression 6/6.5/6.5→8), caught and fixed real bugs worth knowing about: placard accent was clipped by `overflow-hidden`+radius (fixed via a `before:` pseudo-element, not a bordered div), disabled-button `opacity-50` fails contrast badly for informational (non-interactive) disabled states — use `variant="secondary"` + `disabled:opacity-100` instead, `components/ui/accordion.tsx` shipped with a stray `hover:underline` and no hover background (fixed in the shared primitive), and `components/ui/button.tsx`'s `render={<Link/>}` pattern needs `nativeButton={false}` or Base UI throws an a11y console error (now auto-derived in the Button wrapper). Registration funnel (`/register/[slug]`) not built yet — next up.
