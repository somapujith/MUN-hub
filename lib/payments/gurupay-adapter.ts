...
/**
 * `POST /api/create-order` response — confirmed real shape (GuruPay
 * dashboard, 2026-09-18): fields are TOP-LEVEL, not nested under `data`.
 * `{ "status": "success", "payment_url": "https://www.gurupaygateway.com/pay/..." }`
 * No `order_id` echoed back in the confirmed sample — MUN Hub's own
 * generated order_id is authoritative regardless (see `createOrder` below).
 */
...
/**
 * `POST /api/check-status` response — confirmed real shape (GuruPay
 * dashboard, 2026-09-18): fields are TOP-LEVEL, not nested under `data`, and
 * `amount` is a STRING ("100.00"), not a number.
 * `{ "status": "success", "amount": "100.00", "utr": "987654321012", "payment_method": "UPI", "paid_at": "2024-06-05 10:15:30" }`
 * No `order_id`, `currency`, `gateway_txn_id`, `customer_name`/
 * `customer_mobile`, or `provider` field in the confirmed sample — nothing
 * beyond what's shown here is assumed to exist.
 */
interface GuruPayCheckStatusResponse {
  status: GuruPayPaymentStatus
  amount: string
  utr?: string | null
  paid_at?: string | null
}
...
/**
 * Coerces GuruPay's string-typed `amount` ("100.00") to a whole-rupee
 * integer, matching this codebase's amount convention (see
 * lib/payments/adapter.ts's file header / lib/payments/fees.ts) — every
 * `amount` elsewhere in this system is a whole currency unit, never a
 * fractional/float value. Throws on anything that isn't a finite,
 * non-negative number once parsed, so a malformed amount fails loudly here
 * rather than silently producing NaN/an unexpected value that could
 * mis-compare in `processPaymentWebhook`'s `amountMatches` check.
 */
...
function isValidCreateOrderResponse(json: unknown): json is GuruPayCreateOrderResponse {
  return isRecord(json) && typeof json.payment_url === 'string' && typeof json.status === 'string'
}
...
async function createOrder(config: GuruPayAdapterConfig, input: CreateOrderInput): Promise<PaymentOrder> {
  ...
  // The generated order_id is authoritative — create-order's confirmed real
  // response doesn't even echo one back (docs/payments/SPEC.md §4.4.1).
  return { orderId, checkoutUrl: assertValidPaymentUrl(json.payment_url) }
}

/** Maps GuruPay's `status` to the normalized event type, or null for 'pending' (§4.4.1 step 4). */
 ...
 * `${order_id}:${status}:${utr ?? 'none'}` — `order_id` here is always the
 * REQUESTED order id (the one this codebase generated and asked check-status
 * about), never something read off the response, since the confirmed real
 * check-status response doesn't echo `order_id` at all. A later check-status
 * call (webhook retry or the reconciliation job) that returns the same
...
function deriveEventId(requestedOrderId: string, data: GuruPayCheckStatusResponse): string {
  return `${requestedOrderId}:${data.status}:${data.utr ?? 'none'}`
}
 ...
 * confirmed real shape: check-status never echoes `order_id` back, so there
 * is nothing to "verify against the request" beyond using the id we asked
 * with; if a future response shape ever does include an `order_id` field
 * that disagrees with what was requested, that mismatch is exactly the kind
 * of untrusted-provider-data case this system must never silently accept —
 * so it is deliberately ignored in favor of the requested id, not merged in).
 */
function normalizeCheckStatusData(requestedOrderId: string, data: GuruPayCheckStatusResponse): PaymentWebhookEvent | null {
  const type = toEventType(data.status)
  if (type === null) return null

  return {
    eventId: deriveEventId(requestedOrderId, data),
    providerEventType: data.status,
    type,
    providerOrderId: requestedOrderId,
    providerPaymentId: data.utr ?? null,
    amount: parseWholeRupeeAmount(data.amount),
    // Not present in GuruPay's confirmed check-status response at all — this
    // system operates in INR exclusively (see lib/payments/adapter.ts's file
    // header), so it's supplied here rather than trusting an absent field.
    currency: 'INR',
...
function isValidCheckStatusResponse(json: unknown): json is GuruPayCheckStatusResponse {
  if (!isRecord(json)) return false
  return (
    (json.status === 'success' || json.status === 'pending' || json.status === 'failed') &&
...
export async function checkOrderStatus(
  ...
  return normalizeCheckStatusData(orderId, json)
}
