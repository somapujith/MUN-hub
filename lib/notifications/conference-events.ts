...
function buildBody(recipientName: string, munName: string, reason: string | null): string {
  const appUrl = getRuntimeEnv('APP_URL') ?? 'http://localhost:3000'
  const reasonLine = reason ? ` The organizer gave this reason: ${reason}.` : ''

  return (
    `Hi ${recipientName},\n\n` +
    `We're sorry to let you know that "${munName}" has been cancelled.${reasonLine}\n\n` +
 ...
 * only ever called after that transaction commits).
 */
export async function notifyConferenceCancelled(
  ...
  // knows if they did it themselves. `log` can be missing in theory (a
  // CANCELLED transition reached some other way than runLifecycleAction),
  // in which case there's no reviewer to compare against and the organizer
  // is skipped rather than guessed at.
  if (log && log.reviewerId !== mun.organizerId) {
    ...
    recipients.map(async (recipient) => {
      try {
        await adapter.send({
          ...
          body: buildBody(recipient.name, mun.name, reason),
        })
      } catch (error) {
