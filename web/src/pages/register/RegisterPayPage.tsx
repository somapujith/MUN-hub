  ...
  return (
    <div className="flex min-h-full flex-1 flex-col">
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center gap-xl px-lg py-xxl sm:px-xl">
        ...
        {expired ? (
          <RegistrationNotice tone="warning" title="Your seat hold expired" message="Reservations are held for 15 minutes. Start again to reserve a fresh seat.">
            <Button render={<Link to={`/register/${slug}`} />}>Start over</Button>
          </RegistrationNotice>
        ) : (
          <>
            {error === "webhook" && (
              <RegistrationNotice tone="error" title="We couldn't reach the payment processor" message="Your seat is still held and you have not been charged." />
            )}
