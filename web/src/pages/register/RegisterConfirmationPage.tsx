...
function renderStatus(status: RegistrationStatus, slug: string, receipt: ReactNode, registrationId: string) {
  switch (status) {
    ...
    case "CANCELLED":
      return (
        <RegistrationNotice tone="error" title="Payment didn't go through" message="Your seat has been released and you have not been charged." detail={receipt}>
    ...
    case "PENDING":
      return (
        <RegistrationNotice tone="warning" title="Payment still processing" message="We're waiting for the payment provider to confirm." detail={receipt}>
    ...
    default:
      return (
        <RegistrationNotice tone="neutral" title="Registration status" message={`Status: ${STATUS_LABEL[status]}`} detail={receipt}>
          <Button render={<Link to={`/mun/${slug}`} />}>Back to conference</Button>
        </RegistrationNotice>
      );
  }
