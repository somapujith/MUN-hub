...
| `RL_*` (19) | binding | yes | Workers Rate Limiting bindings (`ratelimits` in `wrangler.jsonc`, namespace ids 1001-1019; `env.staging` repeats them as 2001-2019). The newest three are per signed-in user: `RL_SUPPORT_TICKET_USER` (10/min), `RL_SUPPORT_MESSAGE_USER` (30/min), `RL_ACCOUNT_DELETE_USER` (5/min). A missing binding falls back to an in-memory counter per isolate. |
