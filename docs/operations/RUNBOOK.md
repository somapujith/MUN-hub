...
| `RL_*` (18) | binding | yes | Workers Rate Limiting bindings (`ratelimits` in `wrangler.jsonc`, namespace ids 1001-1018; `env.staging` repeats all 18 with ids 2001-2018, because wrangler never inherits `ratelimits` into a named environment). A missing binding falls back to an in-memory counter per isolate. |
