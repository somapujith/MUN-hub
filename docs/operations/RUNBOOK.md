...
| `RL_*` (21) | binding | yes | Workers Rate Limiting bindings (`ratelimits` in `wrangler.jsonc`, namespace ids 1001-1021; `env.staging` repeats all 21 with ids 2001-2021, because wrangler never inherits `ratelimits` into a named environment). Five are per signed-in user: `RL_SUPPORT_TICKET_USER` (10/min), `RL_SUPPORT_MESSAGE_USER` (30/min), `RL_ACCOUNT_DELETE_USER` (5/min), `RL_MFA_MANAGE_USER` (5/min); `RL_FILES_IP` (600/min per IP) stands in for the global cap on uploaded-file reads instead of stacking on it. A missing binding falls back to an in-memory counter per isolate. |
