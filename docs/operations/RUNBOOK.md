...
| `UPLOADS_KV`, `UPLOADS_BUCKET` | binding | for real uploads | Upload storage (`lib/storage/select-adapter.ts`). Both are commented out in `wrangler.jsonc` until the KV namespace / R2 bucket exists; without either (and without `STORAGE_ADAPTER=mock`), uploads are refused with 503 and each attempt logs an error. |
...
| `RL_*` (24) | binding | yes | Workers Rate Limiting bindings (`ratelimits` in `wrangler.jsonc`, namespace ids 1001-1024; `env.staging` repeats all 24 with ids 2001-2024, because wrangler never inherits `ratelimits` into a named environment). Six are per signed-in user: `RL_SUPPORT_TICKET_USER` (10/min), `RL_SUPPORT_MESSAGE_USER` (30/min), `RL_ACCOUNT_DELETE_USER` (5/min), `RL_MFA_MANAGE_USER` (5/min), `RL_UPLOADS_USER` (30/min); `RL_FILES_IP` (1200/min per IP) stands in for the global cap on uploaded-file reads instead of stacking on it. Verification-email resends are capped three ways: `RL_VERIFY_RESEND_IP` (10/min), `RL_VERIFY_RESEND_EMAIL` (3/min) and a per-account cooldown in the action itself. Per-IP limiters key an IPv6 address on its /64 (the smallest block an ISP typically assigns one customer), not the full address. A missing binding falls back to an in-memory counter per isolate. |
